...
public class ShelterController {
	...
	String flist[][] = new String[40][6];
	String l1[] = new String[40];
	String l2[] = new String[40];
	String l3[] = new String[40];
	
	@GetMapping("/search-shelter")
	public String dhundo(Model model) {
        ...
        try {
            ...
            String url = "https://nominatim.openstreetmap.org/search?q=" + URLEncoder.encode(query, "UTF-8") +
                    "&format=json&limit=1000&bounded=1&viewbox=" + URLEncoder.encode(bbox, "UTF-8");
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                while(!queue.isEmpty()) {
                	...
                	l1[i] = list[ind][1];
                	l2[i] = list[ind][2];
                	l3[i] = list[ind][0].substring(0, list[ind][0].indexOf(" ", 30));
                	i++;
                	if(i == p) break;
                }
                for(i=0;i<p;i++) {
        ...
        model.addAttribute("val1", latitude);
        model.addAttribute("val2", longitude);
